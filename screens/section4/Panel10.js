...
const Panel10 = ({ setTimelinePoint }) => {
  ...
  return (
    <div className="w-full min-h-screen relative bg-[url('/images/section4/4.10/bg.webp')] bg-no-repeat bg-cover">
      ...
      >
        <HeaderParallax path={"/images/section4/4.10/ปราสาทบนสวรรค์_"} section="" totalImage={10} parallaxExclude={[]} loader={false} />
        <Button
      ...
      {showCastle && (
        <motion.img
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 2 }}
          className="absolute top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] w-[80%] h-[80%] object-contain drop-shadow"
        />
      )}
    </div>
  )
